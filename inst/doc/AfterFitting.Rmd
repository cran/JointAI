...
iterations. It allows us to visually evaluate convergence and mixing of the chains,
and can be obtained with the function 
...
This allows changing color, linetype and -width, limits, etc.
...
As long as the main parameters have been monitored in a `JointAI` object, 
only these parameters are returned in the model summary, plots and criteria shown above.
...
If a particular chain does not have converged it can be excluded from the result
summary or plot using the argument `exclude_chains` which takes a numeric
vector identifying chains to be excluded, for example:
