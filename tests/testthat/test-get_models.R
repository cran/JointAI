...
test_that("models for compl. long. variables are included ONLY when a
 baseline variable is missing", {
   expect_equal(get_models(fixed = y ~ c1 + C1, random = ~ time | id,
                           data = longDF),
                c(y = 'lmm'))
...
test_that("correct imputation methods are chosen", {
  expect_equal(get_models(fixed = y ~ c1 + C1 + M2 + O2 + C2 + c2,
                          random = ~ b1 + B2 + time | id,
                          data = longDF, no_model = 'time'),
               c(y = 'lmm', c2 = 'lmm', c1 = 'lmm',
                 b1 = 'glmm_binomial_logit',
  ...
  expect_equal(get_models(fixed = y ~ c1 + C1 + o1 + m2 + M2 + O2 + C2 + c2,
                          random = ~ b1 + B2 + time | id,
                          data = longDF, no_model = 'time'),
               c(y = "lmm", c2 = "lmm", m2 = "mlogitmm", c1 = "lmm",
                 o1 = "clmm", b1 = "glmm_binomial_logit", M2 = "mlogit",
                 C2 = "lm", O2 = "clm", B2 = "glm_binomial_logit"))
})
...
test_that("auxvars are included", {
  expect_equal(get_models(fixed = y ~ c1 + C2, auxvars = ~ C1 + B2 + b1,
                          random = ~ time | id, data = longDF,
                          no_model = 'time'),
               c(y = 'lmm', c1 = 'lmm', b1 = 'glmm_binomial_logit',
                 C2 = 'lm', B2 = 'glm_binomial_logit')
  )
})
