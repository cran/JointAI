...
and barplots (for categorical variables) can be obtained.
...
`use_levels = TRUE` and providing the name of the id variable in the argument
`idvar`.
...
```
The information whether a variable is level-1 (i.e., varies within the groups
specified by `idvar`) or level-2 (i.e., constant within the groups) is added to
the title of each plot, and level-2 variables are plotted on the group-level
instead of the observation level.
