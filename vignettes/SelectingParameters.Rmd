...
[`lm_imp()`](https://nerler.github.io/JointAI/reference/model_imp.html), [`glm_imp()`](https://nerler.github.io/JointAI/reference/model_imp.html),
...
[`survreg_imp()`](https://nerler.github.io/JointAI/reference/model_imp.html) and
[`coxph_imp()`](https://nerler.github.io/JointAI/reference/model_imp.html) have an argument
`monitor_params`.
...
(`beta`) and the residual standard deviation (`sigma_y`).
...
```{r lm2_2, fig.height = 2, fig.width = 5, out.width = "60%", message = FALSE}
# Re-run the model from above, now creating MCMC samples
lm2 <- lm_imp(SBP ~ age + WC + alc + smoke + occup,
              data = NHANES, n.iter = 100, progress.bar = 'none',
              monitor_params = c(imps = TRUE, analysis_main = FALSE)
