...
(`beta`) and potentially the residual standard deviation (`sigma_y`), for example
in linear models.
...
Note that instead of `"beta"` the names of the covariates are returned that
are associated with the different elements of `beta`.
...
When `analysis_main` was not switched on the default behavior is that all
