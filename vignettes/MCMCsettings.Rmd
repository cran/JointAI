...
* `parallel`: whether the MCMC chains should be sampled in parallel
* `n.cores`: how many cores are used for parallel sampling
...
results but also when the MCMC chains are sampled in parallel (i.e, when 
...
data, and values where there are missing values in `Xc`, e.g., `Xc[83, 2]` and
`NA` elsewhere.
...
MCMC chains in parallel on multiple cores. This can be specified by setting
the argument `parallel = TRUE`. The maximum number of cores to be used can be
set with the argument `n.cores`. By default, this is two less than the number of
cores available on the machine, but never more than the number of MCMC chains.

Parallel computation is done using the R packages [**foreach**](https://CRAN.R-project.org/package=foreach) and [**doParallel**](https://CRAN.R-project.org/package=doParallel).
Unfortunately, currently it is not possible to obtain a progress bar when using
parallel computation and warning messages produced during the sampling are 
not returned.
