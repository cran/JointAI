...
samples that is used to calculate the summary. The argument `subset` allows controlling
...
This allows changing colour, linetype and -width, limits, etc.
...
allows obtaining predicted values and corresponding
...
via the argument `newdata`. The argument `quantiles` allows specifying the 
...
The function `plot_imp_distr()` allows to visually compare the distribution
