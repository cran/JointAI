...
and [`lme()`](https://stat.ethz.ch/R-manual/R-devel/library/nlme/html/lme.html)
(from package **nlme**).

`lm_imp()` and `glm_imp()` take arguments `formula` and `data`, whereas `lme_imp()`
requires the specification of a `fixed` effects and a `random` effects formula.
Specification of the fixed effects formula is discussed in section [Model formula](#ModelFormula),
specification of the random effects is discussed in section
[Multi level structure & longitudinal covariates](#MultiLevelStructure)

Additionally, `glm_imp()` requires the specification of the model 
...
The arguments `formula` (in `lm_imp()` and `glm_imp()`) and `fixed` 
(in `lme_imp()`) take a two-sided
...
and only the main effects are used as predictors in the imputation models.
...
When the model has many incomplete predictor variables, but not all need to be
changed, it can be useful to perform a set-up run of the model without any 
iterations, extract the vector of imputation models that were automatically 
selected, and apply changes to that vector:
```{r, cache = TRUE}
...
Alternatively, the function [`get_imp_meth()`](https://nerler.github.io/JointAI/reference/get_imp_meth.html)
...
In `lm_imp()`, `glm_imp()` and `lme_imp()`, auxiliary variables can be 
...
or all categorical variabels in the data if only `data` is provided,
