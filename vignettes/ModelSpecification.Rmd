...
to reduce computational time when compiling this vignette.

## Analysis model type
**JointAI** has several main functions:
...
`clm()` and `clmm2()` (from package **ordinal**),
...
variables as extra variables in the dataset, but to them in the model formula.
...
this function is used (as well) in the imputation models.
...
From **JointAI** version 0.5.0 onwards imputation of longitudinal covariates is
possible. Currently, imputation of continuous, binary and ordered factors is 
implemented. For more details, see the section [Imputation model types](#meth)
below.
...
`models` takes a named vector of (imputation) model types, where the names are the names
...
The argument `no_model` (in `get_models()` as well as in the main functions `*_imp()`)
allows to exclude models for such variables (as long as they are completely observed):
```{r}
...
argument `auxvars`, which is a 
vector containing the names or functions of the auxiliary variables.
...
mod9a <- lm_imp(SBP ~ gender + age + occup, auxvars = c('educ', 'smoke'),
                data = NHANES, n.iter = 100, progress.bar = 'none')
```
The `educ` and `smoke` are not used as predictors in the analysis model:
```{r}
summary(mod9a)
```
...
As shown above in [`mod3e`](#mod3e) and `mod3f`, it is possible to specify functions of 
...
reference category. Contrary to the behavior in base R, this is also done for
ordered factors.
...
is returned by **JointAI**. When the argument `keep_scaled_mcmc = TRUE` also
